...
export interface RosterRuntime {
  ...
  moveFollowUps: ReadonlyMap<string, readonly MoveFollowUpDefinition[]>;
...
function mergeUnique<T>(target: Map<string, T>, source: ReadonlyMap<string, T>, label: string): void {
  for (const [key, value] of source) { if (target.has(key)) throw new Error(`duplicate ${label} ${key}`); target.set(key, value); }
}
...
export function compileRosterRuntime(fighterPacksInput: readonly RuntimeFighterPack[], entityPacksInput: readonly RuntimeEntityPack[] = []): RosterRuntime {
  ...
  const attacks = new Map<string, AttackDefinition>(); const moveRuntime = new Map<string, MoveRuntimeDefinition>(); const moveFollowUps = new Map<string, readonly MoveFollowUpDefinition[]>(); const aerialLanding = new Map<string, AerialLandingDefinition>(); const spawns = new Map<string, readonly EntitySpawnDefinition[]>();
  for (const pack of fighterPacks) {
    ...
    mergeUnique(moveFollowUps, compileFighterFollowUps(pack), 'move follow-up');
  ...
  return { fighterDefinitionIds: fighterIds, fighterPhysics: compileFighterPhysicsRegistry(fighterPacks), attacks, grabActions: compileGrabActionRegistry(fighterPacks), moveRuntime, moveFollowUps, aerialLanding, entityDefinitions, entitySpawnsByMoveId: spawns };
}
