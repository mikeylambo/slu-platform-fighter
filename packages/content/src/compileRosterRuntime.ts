import { compileEntityCommands, compileEntityDefinitionRegistry, compileEntitySpawns, type EntityCommandDefinition, type EntityDefinition, type EntitySpawnDefinition } from './compileEntities.js';
...
export interface RosterRuntime {
  ...
  entityDefinitions:ReadonlyMap<string,EntityDefinition>; entitySpawnsByMoveId:ReadonlyMap<string,readonly EntitySpawnDefinition[]>; entityCommandsByMoveId?:ReadonlyMap<string,readonly EntityCommandDefinition[]>;
}
...
export function compileRosterRuntime(fighterPacksInput:readonly RuntimeFighterPack[],entityPacksInput:readonly RuntimeEntityPack[]=[]):RosterRuntime{
  ...
  const attacks=new Map<string,AttackDefinition>();const moveRuntime=new Map<string,MoveRuntimeDefinition>();const moveFollowUps=new Map<string,readonly MoveFollowUpDefinition[]>();const smashCharges=new Map<string,SmashChargePolicy>();const aerialLanding=new Map<string,AerialLandingDefinition>();const spawns=new Map<string,readonly EntitySpawnDefinition[]>();const commands=new Map<string,readonly EntityCommandDefinition[]>();
  for(const pack of fighterPacks){mergeUnique(attacks,compileFighterAttacks(pack),'attack');mergeUnique(moveRuntime,compileFighterMoveRuntime(pack),'move runtime');mergeUnique(moveFollowUps,compileFighterFollowUps(pack),'move follow-up');mergeUnique(smashCharges,compileFighterSmashCharges(pack),'smash charge');mergeUnique(aerialLanding,compileFighterLandingPolicies(pack),'aerial landing policy');mergeUnique(spawns,compileEntitySpawns(pack,entityDefinitions),'entity spawn move');mergeUnique(commands,compileEntityCommands(pack,entityDefinitions),'entity command move');}
  return{fighterDefinitionIds:fighterIds,fighterPhysics:compileFighterPhysicsRegistry(fighterPacks),attacks,grabActions:compileGrabActionRegistry(fighterPacks),moveRuntime,moveFollowUps,smashCharges,aerialLanding,entityDefinitions,entitySpawnsByMoveId:spawns,entityCommandsByMoveId:commands};
}
