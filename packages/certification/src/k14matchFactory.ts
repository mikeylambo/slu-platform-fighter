...
const runtime: RosterRuntime = { fighterDefinitionIds: ['grappler', 'greybox', 'rushdown', 'zoner'], fighterPhysics: new Map(), attacks: new Map(), grabActions: new Map(), moveRuntime: new Map(), moveFollowUps: new Map(), smashCharges: new Map(), aerialLanding: new Map(), entityDefinitions: new Map(), entitySpawnsByMoveId: new Map() };
const stage: CompiledStageDefinition = {
  ...
  spawns: [{ id: 'p1', x: fixed.fromInt(-6), y: fixed.zero, facing: 1 }, { id: 'p2', x: fixed.fromInt(-2), y: fixed.zero, facing: 1 }, { id: 'p3', x: fixed.fromInt(2), y: fixed.zero, facing: -1 }, { id: 'p4', x: fixed.fromInt(6), y: fixed.zero, facing: -1 }],
  stockRules: { blastLeft: fixed.fromInt(-25), blastRight: fixed.fromInt(25), blastBottom: fixed.fromInt(-15), blastTop: fixed.fromInt(20), respawnXSpacing: fixed.fromRatio(3, 2), respawnY: fixed.fromInt(10), respawnFrames: 60, respawnInvulnerableFrames: 120 },
  camera: { left: fixed.fromInt(-18), right: fixed.fromInt(18), bottom: fixed.fromInt(-6), top: fixed.fromInt(14), padding: fixed.fromInt(2), minZoom: fixed.fromRatio(3, 5), maxZoom: fixed.fromRatio(9, 5) }, movingPlatforms: [], hazards: [],
};
const descriptor: StartMatchDescriptor = { mode: 'local-versus', stageId: 'cert-stage', rulesetId: 'teams-stock', teamsEnabled: true, participants: [
...
let rejectedUnknown = false; try { constructMatchFromDescriptor({ ...descriptor, participants: descriptor.participants.map((entry, index) => index === 0 ? { ...entry, fighterId: 'missing' } : entry) }, runtime, stage, 1); } catch { rejectedUnknown = true; }
