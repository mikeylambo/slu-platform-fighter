...
const runtime: RosterRuntime = { fighterDefinitionIds: ['alpha', 'beta'], fighterPhysics: new Map(), attacks: new Map(), grabActions: new Map(), moveRuntime: new Map(), moveFollowUps: new Map(), smashCharges: new Map(), aerialLanding: new Map(), entityDefinitions: new Map(), entitySpawnsByMoveId: new Map() };
const stage: CompiledStageDefinition = {
  ...
  stockRules: { blastLeft: fixed.fromInt(-20), blastRight: fixed.fromInt(20), blastBottom: fixed.fromInt(-12), blastTop: fixed.fromInt(16), respawnXSpacing: fixed.fromInt(2), respawnY: fixed.fromInt(8), respawnFrames: 30, respawnInvulnerableFrames: 60 },
  camera: { left: fixed.fromInt(-15), right: fixed.fromInt(15), bottom: fixed.fromInt(-5), top: fixed.fromInt(12), padding: fixed.fromInt(2), minZoom: fixed.fromInt(1), maxZoom: fixed.fromInt(2) }, movingPlatforms: [], hazards: [],
};
const a = createFighterState('player-1', fixed.fromInt(-2), 1, 'alpha'); const b = createFighterState('player-2', fixed.fromInt(2), -1, 'beta');
const constructed: ConstructedMatch = { world: { frame: 0, seed: 0x4b_19_0001, fighters: [a, b], entities: [], nextEntitySerial: 1, surfaces: stage.surfaces, ledges: stage.ledges, winnerId: null }, runtime, stage, participantControl: { 'player-1': 'human', 'player-2': 'cpu' }, controllerByParticipant: { 'player-1': 'pad-a', 'player-2': null }, paletteByParticipant: { 'player-1': '00', 'player-2': '01' }, teamByParticipant: { 'player-1': 'red', 'player-2': 'blue' }, rulesetId: 'time-one-frame', mode: 'local-versus' };
...
const result = execution.step(execution.initialState, { frame: 0, byFighterId: { 'player-1': neutral(0), 'player-2': neutral(0) } });
