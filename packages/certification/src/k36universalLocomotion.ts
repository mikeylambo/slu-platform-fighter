...
const ledgeAttack:AttackDefinition={id:'greybox:ledge-attack',totalFrames:20,hitboxes:[]}; const attacks=new Map([[ledgeAttack.id,ledgeAttack]]); const captures:WorldState[]=[];
const probe=(state:WorldState,_input:MatchInputFrame):MatchStepResult=>{captures.push(structuredClone(state));return{state:{...state,frame:state.frame+1},events:[]}};
...
step(world,{frame:0,byFighterId:{p1:input(0,{attackPressed:true})}}); const ledgeAttackState=captures.at(-1)?.fighters[0]; assert(ledgeAttackState!==undefined&&ledgeAttackState.grounded&&ledgeAttackState.ledgeId===null&&ledgeAttackState.attack?.attackId==='greybox:ledge-attack','ledge attack input must move fighter onto stage and seed authored ledge attack');
step(world,{frame:0,byFighterId:{p1:input(0,{dodgePressed:true})}}); const roll=captures.at(-1)?.fighters[0]; assert(roll!==undefined&&roll.locomotion==='roll'&&roll.vx<0&&roll.invulnerableFrames>0,'ledge dodge must become inward ledge roll with authored universal invulnerability');
step(world,{frame:0,byFighterId:{p1:input(0,{moveX:-1000})}}); const getup=captures.at(-1)?.fighters[0]; assert(getup!==undefined&&getup.locomotion==='idle'&&getup.grounded&&getup.x<ledge.x,'inward stick must perform neutral getup onto stage');
