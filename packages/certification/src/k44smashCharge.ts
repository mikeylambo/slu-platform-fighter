...
world=step(world,smash(0,1000)).state;assert(Number(world.fighters[0]!.smashCharge?.frames)===1&&world.fighters[0]!.attack===null,'initial smash hold must enter authoritative charge instead of starting attack');
world=step(world,smash(1,1000)).state;assert(Number(world.fighters[0]!.smashCharge?.frames)===2,'held smash must accumulate deterministic charge frames');
const beforeRelease=world;let release=step(world,smash(2,0));world=release.state;const releasedFighter=world.fighters[0] as FighterState;assert(releasedFighter.smashCharge===null&&Number(releasedFighter.attack?.chargeFrames)===3,'release must start ordinary authored smash carrying accumulated charge');
...
const bytesA=serializeWorldState(beforeRelease);const bytesB=serializeWorldState({...beforeRelease,fighters:beforeRelease.fighters.map((fighter,index)=>index===0?{...fighter,smashCharge:{...fighter.smashCharge!,frames:3}}:fighter)});assert(WORLD_BINARY_VERSION===17&&Buffer.from(bytesA).compare(Buffer.from(bytesB))!==0,'binary v17 must hash authoritative smash charge frames');
