...
for (let i = 0; i < RNG_GOLDEN.length; i += 1) {
  rng = nextRngU32(rng);
  if (rng !== RNG_GOLDEN[i]) throw new Error(`K0 RNG golden-vector failure at index ${i}: expected ${RNG_GOLDEN[i]}, got ${rng}`);
}
...
for (let frame = SNAPSHOT_FRAME; frame < TOTAL_FRAMES; frame += 1) {
  ...
  if (actual !== expected) throw new Error(`K0 determinism failure at frame ${frame + 1}: expected ${expected}, got ${actual}`);
}
