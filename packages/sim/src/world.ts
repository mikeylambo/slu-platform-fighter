...
export function createWorld(seed: number): WorldState {
  return { frame: 0, seed, fighters: [createFighterState('fighter-a')], surfaces: createTrainingSurfaces(), ledges: createTrainingLedges() };
}
...
export function stepWorld(state: WorldState, input: SimInputFrame, movementRules: MovementRules = K1_MOVEMENT): WorldState {
  ...
  return { frame: state.frame + 1, seed: state.seed, fighters: [stepFighterMovement(fighter, input, state.surfaces, state.ledges, movementRules)], surfaces: state.surfaces, ledges: state.ledges };
}
