...
export function createFighterState(id: string, x = fixed.fromInt(-10), facing: -1 | 1 = 1, definitionId = 'greybox'): FighterState {
  return {
    ...
    grounded: true, groundSurfaceId: 'ground', facing, locomotion: 'idle', locomotionFrame: 0, jumpsRemaining: 1, fastFalling: false,
    dropThroughFrames: 0, jumpBufferFrames: 0, inputHistory: [], ledgeId: null, ledgeRegrabLockoutFrames: 0, invulnerableFrames: 0, dodgeCooldownFrames: 0, techBufferFrames: 0, landingLagFrames: 0,
    percentTenths: 0, hitlagFrames: 0, hitstunFrames: 0, attack: null, shielding: false, shieldHealth: DEFAULT_SHIELD_HEALTH, shieldStunFrames: 0, shieldRegenDelayFrames: 0,
    recentAttackIds: [], grabTargetId: null, grabbedById: null, grabFrames: 0, grabAction: null, lastHitById: null, lastHitFrame: -1, stocks: DEFAULT_STOCKS, eliminated: false, respawnFrames: 0,
  };
}
export function createStockMatchRuntime(participantIds: readonly string[]): MatchRuntimeState {
  const scores: Record<string, number> = {}; for (const id of [...participantIds].sort()) scores[id] = 0;
  return { mode: 'stock', framesRemaining: null, scores, winningTeamId: null, suddenDeath: false, ended: false };
}
export function createWorld(seed: number): WorldState {
  return { frame: 0, seed, fighters: [createFighterState('fighter-a')], entities: [], nextEntitySerial: 1, items: [], nextItemSerial: 1, surfaces: createTrainingSurfaces(), ledges: createTrainingLedges(), match: createStockMatchRuntime(['fighter-a']), winnerId: null };
}
export function stepWorld(state: WorldState, input: SimInputFrame, movementRules: MovementRules = K1_MOVEMENT): WorldState {
  ...
  const fighter = state.fighters[0]; if (!fighter) throw new Error('K1 world requires fighter-a');
  return {
    frame: state.frame + 1, seed: state.seed, fighters: [stepFighterMovement(fighter, input, state.surfaces, state.ledges, movementRules)],
    entities: state.entities ?? [], nextEntitySerial: state.nextEntitySerial ?? 1,
    items: state.items ?? [], nextItemSerial: state.nextItemSerial ?? 1,
    surfaces: state.surfaces, ledges: state.ledges, ...(state.match ? { match: state.match } : {}), winnerId: state.winnerId,
  };
}
