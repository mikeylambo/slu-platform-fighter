...
export interface FighterState {
  ...
  percentTenths: number; hitlagFrames: number; hitstunFrames: number; attack: FighterAttackState | null; shielding: boolean; shieldHealth: number; shieldStunFrames: number; shieldRegenDelayFrames: number;
...
/** Authoritative item state. Presentation follows holder/socket metadata but never owns item state. */
export interface ItemState {
  id: string; definitionId: string; x: Fixed; y: Fixed; vx: Fixed; vy: Fixed; holderId: string | null; usesRemaining: number; ageFrames: number;
}

export type MatchMode = 'stock' | 'time' | 'stock-time';
export interface MatchRuntimeState {
  mode: MatchMode; framesRemaining: number | null; scores: Record<string, number>; suddenDeath: boolean; ended: boolean; winningTeamId: string | null;
}

export interface WorldState {
  frame: number; seed: number; fighters: FighterState[];
  entities?: OwnedEntityState[]; nextEntitySerial?: number;
  /** Authoritative item collection and monotonic serial. Optional only during legacy-constructor migration. */
  items?: ItemState[]; nextItemSerial?: number;
