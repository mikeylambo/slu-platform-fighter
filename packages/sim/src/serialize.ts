...
export const WORLD_BINARY_VERSION = 2;
...
function writeInput(writer: ByteWriter, input: SimInputFrame) {
  ...
  writer.bool(input.dodgePressed);
  writer.bool(input.shieldHeld);
}
...
function writeLedge(writer: ByteWriter, ledge: StageLedge) {
  writer.string(ledge.id);
  writer.i64(ledge.x);
  writer.i64(ledge.y);
  writer.i16(ledge.inward);
}
...
function writeFighter(writer: ByteWriter, fighter: FighterState) {
  ...
  writer.bool(fighter.ledgeId !== null);
  if (fighter.ledgeId !== null) writer.string(fighter.ledgeId);
  writer.u16(fighter.ledgeRegrabLockoutFrames);
  writer.u16(fighter.invulnerableFrames);
  writer.u16(fighter.dodgeCooldownFrames);
  writer.u16(fighter.techBufferFrames);
...
export function serializeWorldState(state: WorldState): Uint8Array {
  ...
  writer.u16(ledges.length);
  for (const ledge of ledges) writeLedge(writer, ledge);
