...
export const WORLD_BINARY_VERSION = 14;
...
function writeInput(writer: ByteWriter, input: SimInputFrame) {
  ...
  writer.bool(Boolean(input.specialPressed)); writer.bool(Boolean(input.grabPressed));
  writer.i16(input.smashX ?? 0); writer.i16(input.smashY ?? 0);
...
function writeFighter(writer: ByteWriter, fighter: FighterState) {
  ...
  writer.u16(fighter.dodgeCooldownFrames); writer.u16(fighter.techBufferFrames); writer.u16(fighter.landingLagFrames);
  ...
  writer.bool(fighter.grabAction !== null);
  if (fighter.grabAction !== null) {
    writer.string(fighter.grabAction.actionId);
    writer.u16(fighter.grabAction.frame);
  }
  writeOptionalString(writer, fighter.lastHitById);
  writer.i32(fighter.lastHitFrame);
