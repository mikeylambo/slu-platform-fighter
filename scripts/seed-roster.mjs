...
const rigProfilesRoot = join(root, 'content', 'rig-profiles');
...
const manifestPath = isAbsolute(manifestArg) ? manifestArg : resolve(process.cwd(), manifestArg); const manifest = await json(manifestPath); if (!Array.isArray(manifest)) fail('roster manifest must be a JSON array');
const availableRigProfiles = new Set((await readdir(rigProfilesRoot)).filter((name)=>name.endsWith('.json')).map((name)=>name.replace(/\.json$/,'')));
const seen = new Set();
for (const [index, entry] of manifest.entries()) {
  ...
  if (seen.has(entry.id)) fail(`manifest contains duplicate fighter id ${entry.id}`); seen.add(entry.id);
...
for (const entry of manifest) {
  const id = entry.id; const displayName = entry.displayName.trim(); const rigProfile = entry.rigProfile ?? 'slu-humanoid-v1'; const target = join(fightersRoot, id); const alreadyExists = await exists(target);
  if (alreadyExists && !force) { console.log(`SKIP ${id} — fighters/${id} already exists`); skipped += 1; continue; }
  const fighter = structuredClone(baselineTemplate); fighter.id = id; fighter.status = 'draft'; fighter.rigProfile = rigProfile;
